...
const documentRoute = new Hono()
  ...
  .get("all", getAuthUser, async (c) => {
    try {
      ...
      const documents = await db
        ...
        console.log('check it',documents)
      return c.json({
        success: true,
        data: documents,
      });
  ...
  // .leftJoin(personalInfo, eq(documentTable.id, personalInfo.documentId))
  // .leftJoin(experiences, eq(documentTable.id, experiences.documentId))
  // .leftJoin(educations, eq(documentTable.id, educations.documentId))
  // .leftJoin(skills, eq(documentTable.id, skills.documentId))
  .limit(1);
        return c.json({
          success: true,
          data: documentData,
